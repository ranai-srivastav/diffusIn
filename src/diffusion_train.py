...
from models import DiffusionModel, CLIPEncoder, OpenVisionEncoder, ResNet18Encoder
import wandb
import gc
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vision-encoder", type=str, default="OpenVision-vit-tiny",
                       choices=["OpenVision-vit-tiny", "CLIP", "resnet18"],
    ...
    elif args.vision_encoder == "resnet18":
        vision_encoder = ResNet18Encoder(pretrained=False).to(device=device)
    elif args.vision_encoder == "OpenVision-vit-tiny":
        vision_encoder = OpenVisionEncoder().to(device=device)
    else:
        raise ValueError(f"Unknown vision encoder type: {args.vision_encoder}")
    
    args.vision_feature_dim = vision_encoder.feature_dims
